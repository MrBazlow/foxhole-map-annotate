...
function SearchBar (): JSX.Element {
  ...
  const [locations, setLocations] = useState<featureLocation[]>([])

  const olMap = useOlMap()

  const collectLocations = useCallback((currentMap: Map): featureLocation[] | null => {
    const places: featureLocation[] = []
    try {
      currentMap
        .getAllLayers()
        ...
        .forEach((layer) => {
          ...
          if (source instanceof VectorSource) {
            source.forEachFeature((feature) => {
              const coordinates = feature.getGeometry() instanceof Point
  ...
  useEffect(() => {
    if (!toggle || olMap === null) {
      return
    }
    const newCollection = collectLocations(olMap)
    if (newCollection !== null) {
      setLocations(newCollection)
    }
  }, [toggle, olMap, collectLocations])

  const moveMap = useCallback((event: featureLocation) => {
    console.log(event)
    if (olMap === null) {
      return
    }
  ...
  const filteredList = query === ''
    ? []
    : locations.filter((entry) => entry.name
      .toLowerCase()
      .replace(/\s+/g, '')
      .includes(query
        .toLowerCase()
  ...
  return (
    <div role="search" className="relative hidden flex-row-reverse md:flex">
      ...
      <Combobox onChange={(event: featureLocation) => { moveMap(event) }}>
        <Combobox.Input
          type="combobox"
          tabIndex={toggle ? 0 : -1}
          id={inputId}
