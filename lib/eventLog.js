...
const EVENTLOG_FILE = path.join(path.resolve('./data'), 'eventlog.json')
...
class EventLog {
  ...
  rollover = ({ oldData, newData }) => {
    const oldWarDir = path.join(path.resolve('./data'), `war${oldData.warNumber}`)
    // backup old data
